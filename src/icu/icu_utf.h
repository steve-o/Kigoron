...
#ifndef ICU_ICU_UTF_H_
#define ICU_ICU_UTF_H_

#include "chromium/basictypes.hh"

namespace chromium_icu {
 ...
 * @internal
 */
#define CBU8_COUNT_TRAIL_BYTES(leadByte) (chromium_icu::utf8_countTrailBytes[(uint8_t)leadByte])
...
#define CBU8_NEXT(s, i, length, c) { \
    ...
    if(((uint8_t)(c))>=0x80) { \
        if(CBU8_IS_LEAD(c)) { \
            (c)=chromium_icu::utf8_nextCharSafeBody((const uint8_t *)s, &(i), (int32_t)(length), c, -1); \
        } else { \
            (c)=CBU_SENTINEL; \
        } \
    } \
...
#define CBU16_GET_SUPPLEMENTARY(lead, trail) \
    (((chromium_icu::UChar32)(lead)<<10UL)+(chromium_icu::UChar32)(trail)-CBU16_SURROGATE_OFFSET)
...
#define CBU16_LEAD(supplementary) \
    (chromium_icu::UChar)(((supplementary)>>10)+0xd7c0)
...
#define CBU16_TRAIL(supplementary) \
    (chromium_icu::UChar)(((supplementary)&0x3ff)|0xdc00)
...
}  // namesapce chromium_icu

#endif  // ICU_ICU_UTF_H_
