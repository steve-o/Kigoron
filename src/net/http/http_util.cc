...
#include "chromium/strings/string_tokenizer.hh"
...
void HttpUtil::ParseContentType(const std::string& content_type_str,
  ...
  if (param_start != std::string::npos) {
    chromium::StringTokenizer tokenizer(begin + param_start, content_type_str.end(),
                                    ";");
...
bool HttpUtil::ParseRangeHeader(const std::string& ranges_specifier,
  ...
  while (byte_range_set_iterator.GetNext()) {
    ...
    if (!first_byte_pos.empty()) {
      ...
      if (!chromium::StringToInt64(first_byte_pos, &first_byte_position))
        return false;
    ...
    if (!last_byte_pos.empty()) {
      ...
      if (!chromium::StringToInt64(last_byte_pos, &last_byte_position))
        return false;
...
bool HttpUtil::HasHeader(const std::string& headers, const char* name) {
  ...
  std::string::const_iterator it =
      std::search(headers.begin(),
                  ...
                  name,
                  name + name_len,
                  chromium::CaseInsensitiveCompareASCII<char>());
  if (it == headers.end())
    return false;
...
bool HttpUtil::IsSafeHeader(const std::string& name) {
  std::string lower_name(chromium::StringToLowerASCII(name));
...
static const char* FindStatusLineEnd(const char* begin, const char* end) {
  size_t i = chromium::StringPiece(begin, end - begin).find_first_of("\r\n");
  if (i == chromium::StringPiece::npos)
    return end;
  return begin + i;
}
...
std::string HttpUtil::AssembleRawHeaders(const char* input_begin,
  ...
  chromium::CStringTokenizer lines(status_line_end, input_end, "\r\n");
...
std::string HttpUtil::ConvertHeadersBackToHTTPResponse(const std::string& str) {
  ...
  chromium::StringTokenizer tokenizer(str, std::string(1, '\0'));
  while (tokenizer.GetNext()) {
...
std::string HttpUtil::GenerateAcceptLanguageHeader(
  ...
  chromium::StringTokenizer t(raw_language_list, ",");
  ...
  while (t.GetNext()) {
    ...
    if (qvalue10 == 10) {
      ...
      chromium::StringAppendF(&lang_list_with_q, ",%s;q=0.%d", language.c_str(),
                          qvalue10);
    }
...
bool HttpUtil::HasStrongValidators(HttpVersion version,
  ...
  chromium::Time last_modified;
  if (!chromium::Time::FromString(last_modified_header.c_str(), &last_modified))
    return false;

  chromium::Time date;
  if (!chromium::Time::FromString(date_header.c_str(), &date))
    return false;
...
bool HttpUtil::HeadersIterator::AdvanceTo(const char* name) {
  DCHECK(name != NULL);
  DCHECK_EQ(0, chromium::StringToLowerASCII<std::string>(name).compare(name))
