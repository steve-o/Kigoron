...
std::string HttpByteRange::GetHeaderValue() const {
  ...
  if (IsSuffixByteRange())
    return chromium::StringPrintf("bytes=-%" PRId64, suffix_length());

  DCHECK(HasFirstBytePosition());

  if (!HasLastBytePosition())
    return chromium::StringPrintf("bytes=%" PRId64 "-", first_byte_position());

  return chromium::StringPrintf("bytes=%" PRId64 "-%" PRId64,
