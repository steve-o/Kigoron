...
#ifndef NET_HTTP_HTTP_REQUEST_HEADERS_HH_
#define NET_HTTP_HTTP_REQUEST_HEADERS_HH_
...
class HttpRequestHeaders {
 public:
  struct HeaderKeyValuePair {
    HeaderKeyValuePair();
    HeaderKeyValuePair(const chromium::StringPiece& key,
  ...
  bool HasHeader(const chromium::StringPiece& key) const {
  ...
  bool GetHeader(const chromium::StringPiece& key, std::string* out) const;
  ...
  void SetHeader(const chromium::StringPiece& key, const chromium::StringPiece& value);
  ...
  void SetHeaderIfMissing(const chromium::StringPiece& key,
                          const chromium::StringPiece& value);

  // Removes the first header that matches (case insensitive) |key|.
  void RemoveHeader(const chromium::StringPiece& key);
  ...
  // field-content.
  void AddHeaderFromString(const chromium::StringPiece& header_line);
  ...
  void AddHeadersFromString(const chromium::StringPiece& headers);
 ...
 private:
  HeaderVector::iterator FindHeader(const chromium::StringPiece& key);
  HeaderVector::const_iterator FindHeader(const chromium::StringPiece& key) const;
...
#endif  // NET_HTTP_HTTP_REQUEST_HEADERS_HH_
