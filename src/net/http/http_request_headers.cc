...
#include "net/http/http_request_headers.hh"
...
HttpRequestHeaders::HeaderKeyValuePair::HeaderKeyValuePair(
    const chromium::StringPiece& key, const chromium::StringPiece& value)
...
bool HttpRequestHeaders::GetHeader(const chromium::StringPiece& key,
...
void HttpRequestHeaders::SetHeader(const chromium::StringPiece& key,
                                   const chromium::StringPiece& value) {
...
void HttpRequestHeaders::SetHeaderIfMissing(const chromium::StringPiece& key,
                                            const chromium::StringPiece& value) {
  HeaderVector::iterator it = FindHeader(key);
  if (it == headers_.end())
    headers_.push_back(HeaderKeyValuePair(key, value));
...
void HttpRequestHeaders::RemoveHeader(const chromium::StringPiece& key) {
  HeaderVector::iterator it = FindHeader(key);
  if (it != headers_.end())
    headers_.erase(it);
}
...
void HttpRequestHeaders::AddHeaderFromString(
    const chromium::StringPiece& header_line) {
  ...
  const chromium::StringPiece header_key(header_line.data(), key_end_index);
  ...
  if (value_index < header_line.size()) {
    ...
    if (header_value_begin == header_value_end) {
      ...
      SetHeader(header_key,
                chromium::StringPiece(&*header_value_begin,
...
std::string HttpRequestHeaders::ToString() const {
  ...
  for (HeaderVector::const_iterator it = headers_.begin();
       it != headers_.end(); ++it) {
    if (!it->value.empty()) {
      chromium::StringAppendF(&output, "%s: %s\r\n",
                          it->key.c_str(), it->value.c_str());
    } else {
      chromium::StringAppendF(&output, "%s:\r\n", it->key.c_str());
    }
  }
  output.append("\r\n");
  return output;
...
HttpRequestHeaders::FindHeader(const chromium::StringPiece& key) {
  for (HeaderVector::iterator it = headers_.begin();
       it != headers_.end(); ++it) {
    if (key.length() == it->key.length() &&
        !chromium::strncasecmp(key.data(), it->key.data(), key.length()))
      return it;
  }
...
HttpRequestHeaders::FindHeader(const chromium::StringPiece& key) const {
  for (HeaderVector::const_iterator it = headers_.begin();
       it != headers_.end(); ++it) {
    if (key.length() == it->key.length() &&
        !chromium::strncasecmp(key.data(), it->key.data(), key.length()))
