...
#include "chromium/base64.hh"
#include "chromium/logging.hh"
#include "chromium/md5.hh"
#include "chromium/sha1.hh"
...
static uint32_t WebSocketKeyFingerprint(const std::string& str) {
  std::string result;
  ...
  if (spaces == 0)
    return 0;
  int64_t number = 0;
  if (!chromium::StringToInt64(result, &number))
    return 0;
  return htonl(static_cast<uint32_t>(number / spaces));
}
...
class WebSocketHixie76 : public net::WebSocket {
 public:
  ...
  virtual void Accept(const HttpServerRequestInfo& request) override {
    ...
    uint32_t fp1 = WebSocketKeyFingerprint(key1);
    uint32_t fp2 = WebSocketKeyFingerprint(key2);
    ...
    chromium::MD5Digest digest;
    chromium::MD5Sum(data, 16, &digest);
 ...
 private:
  ...
  explicit WebSocketHixie76(HttpConnection* connection,
                   const HttpServerRequestInfo& request,
...
class WebSocketHybi17 : public WebSocket {
 public:
  ...
  virtual void Accept(const HttpServerRequestInfo& request) override {
    ...
    std::string key = request.GetHeaderValue("sec-websocket-key");
    std::string data = chromium::StringPrintf("%s%s", key.c_str(), kWebSocketGuid);
    std::string encoded_hash;
    chromium::Base64Encode(chromium::SHA1HashString(data), &encoded_hash);

    std::string response = chromium::StringPrintf(
        "HTTP/1.1 101 WebSocket Protocol Handshake\r\n"
        "Upgrade: WebSocket\r\n"
        "Connection: Upgrade\r\n"
 ...
 private:
  explicit WebSocketHybi17(HttpConnection* connection,
                  const HttpServerRequestInfo& request,
                  size_t* pos)
