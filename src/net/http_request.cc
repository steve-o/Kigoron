...
void HttpRequestParser::ProcessChunk(const chromium::StringPiece& data) {
...
HttpRequestParser::ParseResult HttpRequestParser::ParseHeaders() {
  ...
  // Request main main header, eg. GET /foobar.html HTTP/1.1
  {
    ...
    chromium::SplitString(header_line, ' ', &header_line_tokens);
    DCHECK_EQ(3u, header_line_tokens.size());
    // Method.
    http_request_->method = GetMethodType(chromium::StringToLowerASCII(header_line_tokens[0]));
    // Address.
    ...
    // Protocol.
    const std::string protocol = chromium::StringToLowerASCII(header_line_tokens[2]);
  ...
  if (http_request_->headers.count("Content-Length") > 0) {
    ...
    const bool success = chromium::StringToSizeT(
...
std::shared_ptr<HttpRequest> HttpRequestParser::GetRequest() {
  DCHECK_EQ(STATE_ACCEPTED, state_);
  std::shared_ptr<HttpRequest> result = std::move (http_request_);
