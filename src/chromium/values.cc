...
#include "chromium/float_util.hh"
...
bool DictionaryValue::Get(const std::string& path, const Value** out_value) const {
//  DCHECK(IsStringUTF8(path));
  std::string current_path(path);
...
bool DictionaryValue::Get(const std::string& path, Value** out_value)  {
  return static_cast<const DictionaryValue&>(*this).Get(path, const_cast<const Value**>(out_value));
}
...
bool DictionaryValue::GetBinary(const std::string& path,
  ...
  return static_cast<const DictionaryValue&>(*this).GetBinary(path, const_cast<const BinaryValue**>(out_value));
}
...
bool DictionaryValue::GetDictionary(const std::string& path,
  ...
  return static_cast<const DictionaryValue&>(*this).GetDictionary(path, const_cast<const DictionaryValue**>(out_value));
}
...
bool DictionaryValue::GetList(const std::string& path, ListValue** out_value) {
  return static_cast<const DictionaryValue&>(*this).GetList(path, const_cast<const ListValue**>(out_value));
}
...
bool DictionaryValue::GetWithoutPathExpansion(const std::string& key,
                                              Value** out_value) {
  return static_cast<const DictionaryValue&>(*this).GetWithoutPathExpansion(key, const_cast<const Value**>(out_value));
}
...
bool ListValue::GetDictionary(size_t index, const DictionaryValue** out_value) const {
  const Value* value;
  bool result = Get(index, &value);
  if (!result || !value->IsType(TYPE_DICTIONARY))
    return false;
