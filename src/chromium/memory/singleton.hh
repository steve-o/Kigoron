...
struct StaticMemorySingletonTraits {
  ...
  static Type* New() {
    if (chromium::subtle::NoBarrier_AtomicExchange(&dead_, 1))
      return NULL;
  ...
  static void Delete(Type* p) {
    chromium::subtle::NoBarrier_Store(&dead_, 1);
    chromium::subtle::MemoryBarrier();
    if (p != NULL)
      p->Type::~Type();
  }
  ...
  static void Resurrect() {
    chromium::subtle::NoBarrier_Store(&dead_, 0);
  }
 ...
 private:
  ...
  static chromium::subtle::Atomic32 dead_;
};

template <typename Type> intptr_t
    StaticMemorySingletonTraits<Type>::buffer_[kBufferSize];
template <typename Type> chromium::subtle::Atomic32
...
//
// In your header:
//   #include "chromium/memory/singleton.h"
//   class FooClass {
//    public:
...
class Singleton {
 private:
  ...
  static Type* get() {
    chromium::subtle::AtomicWord value = chromium::subtle::NoBarrier_Load(&instance_);
    if (value != 0 && value != chromium::internal::kBeingCreatedMarker) {
      return reinterpret_cast<Type*>(value);
    }

    // Object isn't created yet, maybe we will get to create it, let's try...
    if (chromium::subtle::Acquire_CompareAndSwap(
          &instance_, 0, chromium::internal::kBeingCreatedMarker) == 0) {
      // instance_ was NULL and is now kBeingCreatedMarker.  Only one thread
      // will ever get here.  Threads might be spinning on us, and they will
      // stop right after we do this store.
      Type* newval = Traits::New();

      chromium::subtle::Release_Store(
          &instance_, reinterpret_cast<chromium::subtle::AtomicWord>(newval));

      return newval;
    }

    // We hit a race. Wait for the other thread to complete it.
    value = chromium::internal::WaitForInstance(&instance_);
  ...
  static void OnExit(void* /*unused*/) {
    ...
    Traits::Delete(
        reinterpret_cast<Type*>(chromium::subtle::NoBarrier_Load(&instance_)));
    instance_ = 0;
  }
  static chromium::subtle::AtomicWord instance_;
};

template <typename Type, typename Traits, typename DifferentiatingType>
chromium::subtle::AtomicWord Singleton<Type, Traits, DifferentiatingType>::
    instance_ = 0;

#endif  // CHROMIUM_MEMORY_SINGLETON_HH__
